...
export class EditCategory {
  ...
  categoryResource = this.categoryResourceRef.value;
  ...
  effectRef = effect(() => {
    this.editCategoryFormGroup.controls.name.patchValue(this.categoryResource()?.name ?? '');
    this.editCategoryFormGroup.controls.urlHandle.patchValue(
      this.categoryResource()?.urlHandle ?? ''
    );
  });
